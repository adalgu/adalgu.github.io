...
class NotionSetup:
        ...
        if self.parent_page_id:
            ...
            try:
                ...
                if "parent.type" in str(e) or "parent.page_id" in str(e):
                    raise ValueError(
                        "워크스페이스 루트에 데이터베이스를 생성할 권한이 없습니다. "
                        "상위 페이지 ID를 지정하거나 통합 권한을 확인하세요."
                    ) from e
                raise
    ...
    def create_sample_post(self, database_id: str) -> Dict[str, Any]:
        ...
        properties = {
            ...
            "expiryDate": {"date": {"start": None}},  # 만료 날짜 없음
            # 메타데이터 속성 (추천)
            "Description": {
                "rich_text": [
                    {
...
class NotionMigration(NotionSetup):
            ...
            for page in pages:
                ...
                try:
                    ...
                    if properties is None:
                        print(f"페이지 {page['id']} 변환 건너뜀(skipRendering)")
                        stats["skipped"] += 1
                        continue
    ...
    def _transform_properties(self, properties: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 만약 skipRendering 또는 doNotRendering 속성이 체크되어 있다면, 이 페이지의 변환을 건너뜁니다.
        if (properties.get("skipRendering", {}).get("checkbox", False) == True) or (
            properties.get("doNotRendering", {}).get("checkbox", False) == True
        ):
            return None
        ...
        if (
            "Date" in properties
            and properties["Date"]["type"] == "date"
            and properties["Date"].get("date")
        ):
            transformed["Date"] = {"date": properties["Date"]["date"]}
        else:
            date_found = False
            for prop_name, prop in properties.items():
                if (
                    prop["type"] == "date"
                    and prop_name not in ["Date", "Created time"]
                    and prop.get("date")
                ):
            ...
            if not date_found:
                if (
                    ...
                    and properties["Created time"].get("date")
                ):
